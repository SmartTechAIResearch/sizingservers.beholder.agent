...
namespace sizingservers.beholder.agent.shared {
    public class Config {
        ...
        public int requestReportTcpPort { get; internal set; }
            ...
            foreach (var kvp in kvps)
                switch (kvp.Key) {
                    ...
                    case "reportevery":
                        ...
                        break;
                    case "requestreporttcpport":
                        requestReportTcpPort = int.Parse(kvp.Value);
                        break;
                }
        }
        ...
        private Dictionary<string, string> GetConfKvps() {
            ...
            using (var sr = new StreamReader(
                new FileStream(
                    Path.Combine(AppContext.BaseDirectory, "sizingservers.beholder.agent.conf"), FileMode.Open)))
                while (sr.Peek() != -1) {
